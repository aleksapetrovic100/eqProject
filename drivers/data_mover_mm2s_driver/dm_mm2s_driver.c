...
static ssize_t dm_mmap(struct file *f, struct vm_area_struct *vma_s)
{
	...
	printk(KERN_INFO "DM TX Buffer is being memory mapped\n");
...
u32 dm_simple_write(dma_addr_t TxBufferPtr, u32 max_pkt_len, void __iomem *base_address) {
  ...
  u32 BytesToTransfer = max_pkt_len;

  printk("BytesToTransfer %d SADDR %d \n",BytesToTransfer,SADDR);

  iowrite32(1<<30 | 1<<23 | 23 | BytesToTransfer, base_address); //SLANJE KOMANDE NA DATAMOVER HELPER KOJI KONTROLISE DATAMOVER u 3 registra jer je komandna rec 72 bita
  iowrite32(SADDR, base_address+1); 
  iowrite32((u32)DM_EN, base_address+2);

  while(ioread32(base_address+3) && (u32)1 == 0) // Da li moze while na ovom mestu da se primeni
  {
...
static int __init dm_init(void)
{
  ...
  if (device_create(cl, NULL, MKDEV(MAJOR(first),0), NULL, "dm_mm2s") == NULL)
  {
    goto fail_1;
  }
  ...
  printk(KERN_INFO "DM memory reset.\n");
