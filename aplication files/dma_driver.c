...
static ssize_t vga_dma_write(struct file *f, const char __user *buf, size_t count, loff_t *off) // IZ buf IZVLACI PODATKE O xpos, ypos i rgb, buf se jedino pojavljuje u funkcijama read i write
{
...
static ssize_t vga_dma_mmap(struct file *f, struct vm_area_struct *vma_s) // OVDE VIRTUELNI PODATCI U PRAVE PODATKE, mozemo ostaviti "istu"
{
	int ret = 0;
